...
| 2025-11-21 | UI 重构 & AI 词典 | **UI 重构**：采用 Apple 风格设计（毛玻璃、圆角、阴影），实现响应式双栏布局；**AI 词典**：实现基于 LLM 的上下文查词功能，支持 JSON 结构化输出与本地缓存（`~/.cache/lingoview`）；**工程优化**：修复 CI 依赖与 Labeler 配置，确保自动化测试通过 | 词典缓存目前仅为本地 JSON 文件，大量数据下性能需评估；前端组件拆分后需持续维护类型定义 | 进一步优化移动端适配；考虑引入 SQLite 替代 JSON 缓存；探索更多本地 LLM 查词方案 |
